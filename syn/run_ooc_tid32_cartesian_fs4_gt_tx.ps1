...
$outDir = Join-Path $OutRoot "tid32_cartesian_fs4_gt_tx_ooc_$($Part.Replace('-','_'))_${label}mhz_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
& $VivadoBat -mode batch -source (Join-Path $PSScriptRoot 'run_ooc_tid32_cartesian_fs4_gt_tx.tcl') -tclargs $Part $TargetMHz $outDir
if ($LASTEXITCODE -ne 0) { throw "TID32 Cartesian GT OOC failed: exit=$LASTEXITCODE" }
...
if ($row.Status -ne 'PASS') { throw "TID32 Cartesian GT OOC timing failed: $($row.Status)" }
