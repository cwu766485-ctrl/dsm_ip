...
file mkdir $out_dir
cd $out_dir
# Keep this OOC flow aligned with the proven local TID32 OOC scripts.  All
# reports/checkpoints are written under out_dir; the in-memory project avoids
# project-local cache state from influencing the timing measurement.
...
foreach file [list \
  ...
  [file join $root syn rtl tid32_thermo3_frontend_tx_ooc.sv]] { read_verilog -sv $file }
set_param general.maxThreads 1
set_param synth.maxThreads 1
synth_design -top tid32_thermo3_frontend_tx_ooc -part $part -mode out_of_context -flatten_hierarchy none -directive AreaOptimized_high
...
puts $f "Part,Block,Target_MHz,Status,WNS_ns,WHS_ns"
puts $f "$part,tid32_thermo3_frontend_tx_ooc,[format %.2f $target_mhz],$status,[format %.3f $wns],[format %.3f $whs]"
close $f
puts "TID32_THERMO3_FRONTEND_OOC_STATUS=$status"
