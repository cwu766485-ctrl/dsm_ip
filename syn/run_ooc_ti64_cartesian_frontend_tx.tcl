...
read_verilog -sv [file join $root rtl interp dsm_interp_frontend.sv]
...
read_verilog -sv [file join $root rtl tx_bandpass_if ti64_cartesian_frontend_tx.sv]
read_verilog -sv [file join $root syn rtl ti64_cartesian_frontend_tx_ooc.sv]
set_param general.maxThreads 1
set_param synth.maxThreads 1
synth_design -top ti64_cartesian_frontend_tx_ooc -part $part -mode out_of_context -flatten_hierarchy none -directive AreaOptimized_high
write_checkpoint -force [file join $out_dir synthesized.dcp]
create_clock -name clk -period $period_ns [get_ports clk]
# Fabric-only result. The real GT clock, I/O delays, reset and CDC constraints
# belong in the generated GT Wizard and board top; do not use zero-delay I/O.
...
puts $fp "$part,ti64_cartesian_frontend_tx_ooc,[format %.2f $target_mhz],$status,[format %.3f $wns],[format %.3f $whs],[format %.2f $fmax]"
close $fp
puts "TI64_CARTESIAN_FRONTEND_TX_OOC_STATUS=$status"
