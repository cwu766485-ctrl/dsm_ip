...
set out_root [file normalize [file join $script_dir "reports" "routed_dsm_ip_axi_${part_tag}_${stamp}"]]
file mkdir $out_root
...
set cases [list \
  [list 2 0 ef_dsm_bypass_default] \
  [list 6 4 mash22_native_x32_worst_dsp] \
  [list 12 4 mash111_multibit_x32_max_lut] \
]

set summary_csv [file join $out_root "summary_subset.csv"]
set fp [open $summary_csv "w"]
puts $fp "Part,Top,Algorithm,AlgorithmName,InterpMode,InterpName,DucMode,Status,LUT,FF,DSP,WNS_ns,TNS_ns,Fmax_est_MHz"

foreach item $cases {
  lassign $item alg interp case_name
  set top_tag "dsm_ip_axi_${case_name}"
  puts "==== Routed OOC $top_tag ===="
  ...
  puts $fp "$part,$top_tag,$alg,[dict get $alg_names $alg],$interp,[dict get $interp_names $interp],0,$status,$lut,$ff,$dsp,$wns,$tns,$fmax"
  flush $fp
  close_project
}

close $fp
puts "DSM IP AXI routed OOC subset summary: $summary_csv"
