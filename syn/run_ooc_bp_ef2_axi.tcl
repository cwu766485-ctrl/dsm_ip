# 100 MHz FPGA OOC synthesis of the complete DPD -> interpolation ->
# full-precision IF mixer -> one-bit BP EFDSM2 AXI transmitter SKU.
...
set out_dir [file normalize [file join $script_dir reports "bp_ef2_axi_ooc_${tag}_${stamp}"]]
file mkdir $out_dir
cd $out_dir
puts "BP EFDSM2 AXI OOC: part=$part out_dir=$out_dir"
...
set fmax [expr {1000.0 / (10.0 - double($wns))}]
set status "PASS"
if {$wns < 0.0} { set status "FAIL_TIMING" }
set fp [open [file join $out_dir summary.csv] w]
puts $fp "Part,SKU,Algorithm,DucMode,InterpMode,Status,WNS_ns,Fmax_est_MHz"
puts $fp "$part,DPD_Interp_BP_EFDSM2,3,3,4,$status,[format %.3f $wns],[format %.2f $fmax]"
close $fp
puts "BP EFDSM2 AXI OOC summary: [file join $out_dir summary.csv]"
if {$wns < 0.0} { error "BP EFDSM2 AXI SKU fails 100 MHz: WNS=$wns" }
