...
$outDir = Join-Path $OutRoot "tid32_thermo5_fs4_multipa_tx_ooc_$($Part.Replace('-','_'))_${label}mhz_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
& $VivadoBat -mode batch -source (Join-Path $PSScriptRoot 'run_ooc_tid32_thermo5_fs4_multipa_tx.tcl') -tclargs $Part $TargetMHz $outDir
if ($LASTEXITCODE -ne 0) { throw "TID32 thermo5 OOC failed: exit=$LASTEXITCODE" }
...
if ($row.Status -ne 'PASS') { throw "TID32 thermo5 OOC timing failed: $($row.Status)" }
