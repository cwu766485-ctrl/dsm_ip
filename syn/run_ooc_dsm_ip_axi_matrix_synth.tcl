...
set out_root [file normalize [file join $script_dir "reports" "synth_dsm_ip_axi_${part_tag}_${stamp}"]]
file mkdir $out_root
...
puts $fp "Part,Flow,Top,Algorithm,AlgorithmName,InterpMode,InterpName,DucMode,Status,LUT,FF,DSP,WNS_ns,Fmax_est_MHz"

for {set alg 0} {$alg <= 13} {incr alg} {
  for {set interp 0} {$interp <= 4} {incr interp} {
    set top_tag "dsm_ip_axi_alg${alg}_interp${interp}"
    puts "==== SYNTH OOC $top_tag ===="
    ...
    puts $fp "$part,post_synth_ooc,$top_tag,$alg,[dict get $alg_names $alg],$interp,[dict get $interp_names $interp],0,$status,$lut,$ff,$dsp,$wns,$fmax"
    flush $fp
    close_project
  }
}
...
close $fp
puts "DSM IP AXI post-synthesis OOC matrix summary: $summary_csv"
