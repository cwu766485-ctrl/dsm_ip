...
foreach f [glob -nocomplain [file join $root rtl dpd *.v]] { read_verilog $f }
read_verilog -sv [file join $root rtl dpd dpd_vector16_frontend.sv]
read_verilog -sv [file join $root rtl interp dsm_interp_x4_polyphase16.sv]
read_verilog -sv [file join $root rtl tx_bandpass_if ti32_lp1_fs4_dsm.sv]
read_verilog -sv [file join $root rtl gt gt_tx_user_bridge.sv]
read_verilog -sv [file join $root rtl gt gt_tx_raw64_boundary.sv]
read_verilog -sv [file join $root rtl tx_bandpass_if ti64_lp1_fs4_gt_tx.sv]
read_verilog -sv [file join $root rtl tx_bandpass_if ti64_cartesian_x4_frontend_tx.sv]
read_verilog -sv [file join $root syn rtl ti64_cartesian_x4_frontend_tx_ooc.sv]
set_param general.maxThreads 1
set_param synth.maxThreads 1
synth_design -top ti64_cartesian_x4_frontend_tx_ooc -part $part -mode out_of_context -flatten_hierarchy none -directive AreaOptimized_high
write_checkpoint -force [file join $out_dir synthesized.dcp]
create_clock -name clk -period $period_ns [get_ports clk]
# Fabric-only result. GT hard-IP, I/O, reset and CDC constraints stay in the
# board integration; this task makes no board or RF timing claim.
...
puts $fp "$part,ti64_cartesian_x4_frontend_tx_ooc,[format %.2f $target_mhz],$status,[format %.3f $wns],[format %.3f $whs],[format %.2f $fmax]"
close $fp
puts "TI64_CARTESIAN_X4_FRONTEND_TX_OOC_STATUS=$status"
