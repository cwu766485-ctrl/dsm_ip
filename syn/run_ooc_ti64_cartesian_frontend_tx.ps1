...
$outDir = Join-Path $OutRoot "ti64_cartesian_frontend_tx_ooc_$($Part.Replace('-','_'))_${label}mhz_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
& $VivadoBat -mode batch -source (Join-Path $PSScriptRoot 'run_ooc_ti64_cartesian_frontend_tx.tcl') -tclargs $Part $TargetMHz $outDir
if ($LASTEXITCODE -ne 0) { throw "TI64 Cartesian frontend OOC failed: exit=$LASTEXITCODE" }
