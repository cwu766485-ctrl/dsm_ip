...
$outDir = Join-Path $OutRoot "ti32_lp1_fs4_dsm_ooc_$($Part.Replace('-','_'))_${label}mhz_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
& $VivadoBat -mode batch -source (Join-Path $PSScriptRoot 'run_ooc_ti32_lp1_fs4_dsm.tcl') -tclargs $Part $TargetMHz $outDir
if ($LASTEXITCODE -ne 0) { throw "TI32 OOC failed: exit=$LASTEXITCODE" }
