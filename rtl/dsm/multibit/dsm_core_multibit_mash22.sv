...
module dsm_core_multibit_mash22 #(
  ...
  dsm_core_multibit #(.W_IN(W_IN), .ACC_W(ACC_W), .OUT_W(OUT_W), .Q_BITS(Q_BITS),
    .MB_ALGORITHM(6), .IN_SHIFT(IN_SHIFT), .SATURATE(SATURATE),
    .COEFF_W(COEFF_W), .B1_NUM(B1_NUM), .B2_NUM(B2_NUM), .COEFF_SHIFT(COEFF_SHIFT)) u_core (
