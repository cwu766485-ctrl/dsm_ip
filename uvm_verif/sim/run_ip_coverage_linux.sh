...
python3.12 uvm_verif/sim/run_regression.py \
  --sim vcs \
  --tests dsm_bp_test,dsm_performance_bittrue_test,dsm_memory_dpd_bittrue_test,dsm_memory_dpd_safety_test \
  --seeds 1 \
  --jobs 2 \
...
python3.12 uvm_verif/sim/run_regression.py \
  --sim vcs \
  --tests dsm_axis_coverage_test \
  --seeds 1,7 \
  --jobs 2 \
  --summary uvm_verif/sim/out/vcs/ip_coverage_axis.csv \
  --skip-compile

{
  ...
  tail -n +2 uvm_verif/sim/out/vcs/ip_coverage_axis.csv
...
dsm_axis_coverage_test_seed1 dsm_axis_coverage_test_seed7"
