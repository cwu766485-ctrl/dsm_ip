...
// This test uses the same bit-true Memory-Poly5 path as the deterministic
// testcase, while independently randomising every AXI-Lite channel delay.
// The input vector and RF oracle remain deterministic; only control-plane
// arrival/response timing changes across seeds.
class dsm_memory_dpd_commit_stress_test extends dsm_base_test;
  `uvm_component_utils(dsm_memory_dpd_commit_stress_test)
