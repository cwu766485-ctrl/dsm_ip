...
$work = Join-Path $repo 'verif\out_xsim_ti64_cartesian_frontend_tx'
...
$sources += @(
  ...
  (Join-Path $repo 'rtl\tx_bandpass_if\ti64_cartesian_frontend_tx.sv'),
  (Join-Path $repo 'verif\block\bp_dsm\tb\tb_ti64_cartesian_frontend_tx.sv')
)
Push-Location $work
try {
  Get-ChildItem -Force | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
  $sourceArgs = ($sources | ForEach-Object { '"' + $_ + '"' }) -join ' '
  $batch = "call `"$settings`" >nul && cd /d `"$work`" && xvlog -sv $sourceArgs > console_xvlog.log 2>&1 && xelab tb_ti64_cartesian_frontend_tx -s sim_ti64_cartesian > console_xelab.log 2>&1 && xsim sim_ti64_cartesian -runall > console_xsim.log 2>&1"
  & cmd.exe /d /c $batch
  if ($LASTEXITCODE -ne 0) {
    Get-ChildItem -Filter '*.log' | ForEach-Object { Get-Content $_.FullName -Tail 100 }
    throw "TI64 Cartesian frontend XSim failed: $LASTEXITCODE"
  }
} finally {
  Pop-Location
}
if (!(Select-String -Path "$work\console_xsim.log" -Pattern 'TI64_CARTESIAN_FRONTEND_TX_PASS' -Quiet)) {
  throw 'TI64 Cartesian frontend PASS marker missing'
}
Write-Host "TI64 Cartesian frontend XSim PASS: $work\console_xsim.log"
