...
module tb_bp_ef2_map_compose_ctx_pipe;
  ...
  initial begin
    ...
    while (drive_index < TRANSACTIONS) begin
      if (drive_index == (TRANSACTIONS/2)) begin
        @(negedge clk);
        rst_n = 1'b0; in_valid = 1'b0; left_x = '0; right_x = '0;
        repeat (3) @(negedge clk);
        rst_n = 1'b1;
        // Reset intentionally flushes transactions still in flight.  The next
        // output must therefore correspond to the first post-reset input.
        check_index = drive_index;
      end
      @(negedge clk);
      cycle = cycle + 1;
      if (((cycle % 7) == 3) || ((cycle % 11) == 5)) begin
        in_valid = 1'b0; left_x = '0; right_x = '0;
      end else begin
    ...
    @(negedge clk); in_valid = 1'b0; left_x = '0; right_x = '0;
    repeat (LATENCY + 12) @(negedge clk);
    if (check_index != TRANSACTIONS) $fatal(1, "lost outputs %0d/%0d", check_index, TRANSACTIONS);
    if (errors != 0) $fatal(1, "compose-ctx-pipe errors=%0d", errors);
    $display("BP_EF2_MAP_COMPOSE_CTX_PIPE_PASS transactions=%0d latency=%0d", TRANSACTIONS, LATENCY);
    $finish;
  end
  ...
  always @(posedge clk) begin
    ...
    if (rst_n && pipe_valid) begin
      if (check_index >= TRANSACTIONS) begin
        $error("unexpected extra output"); errors = errors + 1;
      end else if ((pv !== exp_v[check_index]) || (ps !== exp_s[check_index]) || (plo !== exp_lo[check_index]) || (phi !== exp_hi[check_index]) || (pof !== exp_of[check_index]) || (pb !== exp_b[check_index])) begin
        $error("compose context pipeline mismatch transaction=%0d", check_index); errors = errors + 1;
      end
