...
module tb_dpd_poly;
  ...
  initial begin
    ...
    if (sample_count != N_IN) $fatal(1, "DPD sample_count mismatch");
    $fclose(fout);
    $display("TB DPD poly done: %0d samples saturation_count=%0d", out_count, saturation_count);
    $finish;
  end
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (cycle_count > MAX_CYCLES) $fatal(1, "DPD bit-true timeout");
