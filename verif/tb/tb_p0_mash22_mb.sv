...
module tb_p0_mash22_mb;
  ...
  initial begin
    out_yout = plusarg_or_default("OUT_YOUT", "sim_yout_signed_mash22_mb.txt");
    out_summary = plusarg_or_default("OUT_SUMMARY", "summary_tb_p0_mash22_mb.csv");
    rst_n = 1'b0;
    enable = 1'b0;
    use_nco = 1'b0;
    ...
    $fclose(f_yout);
    tb_write_summary(out_summary, "tb_p0_mash22_mb", bb_count);
    $display("TB P0 MASH22_MB done: %0d samples", bb_count);
    if (tb_error_count != 0) $fatal(1, "TB P0 MASH22_MB self-check failed with %0d errors", tb_error_count);
    $finish;
  end
