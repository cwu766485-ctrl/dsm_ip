...
module tb_p0_ef2;
  ...
  initial begin
    out_bits = plusarg_or_default("OUT_BITS", "sim_bits_01_ef2.txt");
    out_summary = plusarg_or_default("OUT_SUMMARY", "summary_tb_p0_ef2.csv");
    rst_n = 1'b0;
    enable = 1'b0;
    use_nco = 1'b0;
    ...
    $fclose(f_bits);
    tb_write_summary(out_summary, "tb_p0_ef2", bb_count);
    $display("TB P0 EF2 done: %0d samples", bb_count);
    if (tb_error_count != 0) $fatal(1, "TB P0 EF2 self-check failed with %0d errors", tb_error_count);
    $finish;
  end
