%% ============================================================
% stage1_ofdm_qam_to_coe_lp_dsm_input_256qam.m
% Stage-1: bits -> OFDM-256QAM baseband IQ
%        -> interpolation (OSR) -> fixed-point (Q1.15)
%        -> export Vivado .coe for I/Q ROM (dual-path LPDSM input)
%
% NOTE: This is a 256QAM-specific variant. It keeps the same interface
% and meta fields as stage1_ofdm_qam_to_coe_lp_dsm_input_v3.m.
%% ============================================================

function [fnameI, fnameQ, metaFile] = stage1_ofdm_qam_to_coe_lp_dsm_input_256qam(OSR, ROM_DEPTH, enable_edge_taper)
% Usage:
% [fi,fq,meta] = stage1_ofdm_qam_to_coe_lp_dsm_input_256qam(32,65536,false);
...
active_bins = [-26:-1, 1:26];
used_sc = zeros(1, numel(active_bins));
for scIdx = 1:numel(active_bins)
    b = active_bins(scIdx);
    if b == 0
        used_sc(scIdx) = 1;
    elseif b > 0
...
figure('Name','256-QAM Constellation');
plot(qam_data(:), '.'); grid on; axis square;
title('256-QAM Constellation'); xlabel('I'); ylabel('Q');
...
% Optional edge taper (WOLA-like)
if enable_edge_taper
    L = Nfft+Ncp;
    Nt = min(8, floor(L/8));
    w = ones(L,1);
    r = (0:Nt-1)'/Nt;
    taper = 0.5*(1-cos(pi*r));
    w(1:Nt) = taper;
...
function s = qammod_local_gray(bits, M)
    ...
    % Map to PAM levels: -(L-1), ..., +(L-1) step 2
    levels = -(L-1):2:(L-1);
    xi = levels(idxI+1);
    xq = levels(idxQ+1);
...
function idx = gray_to_bin(b)
    % b: [N x k] Gray-coded bits, MSB first
    k = size(b,2);
    bin = zeros(size(b));
    bin(:,1) = b(:,1);
    for bitIdx = 2:k
