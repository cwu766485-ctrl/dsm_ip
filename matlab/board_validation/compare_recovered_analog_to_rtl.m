function compare_recovered_analog_to_rtl()
% compare_recovered_analog_to_rtl
% Reconstruct analog baseband waveforms from:
%   1) recovered 0/1 bitstreams from scope CSV
%   2) matched RTL rf_bit segments
% Then compare them under the same offline DSP flow.
%
% This is intended to support the statement:
% "The recovery is valid in the analog-signal-reconstruction sense",
% without claiming strict global bit-true equivalence.
...
for k = 1:numel(caps)
    ...
    title(sprintf('%s: recovered-analog vs RTL-analog spectrum | corr=%.4f', ...
        caps(k).name, bb_psd_corr), 'Interpreter', 'none');
    ...
    ylabel('Amplitude');
    title(sprintf('%s: aligned reconstructed analog | corr=%.4f | EVM-like=%.2f%%', ...
        caps(k).name, wave_corr, evm_pct), 'Interpreter', 'none');
    legend({'recovered I', 'RTL I', 'recovered Q', 'RTL Q'}, 'Location', 'best');
end

sgtitle(tlo, 'Recovered Analog Waveform vs RTL Analog Waveform');
exportgraphics(fig, fullfile(repo, 'recovered_analog_vs_rtl_baseband.png'), 'Resolution', 180);
close(fig);

T = cell2table(rows, 'VariableNames', ...
    {'capture', 'evm_percent', 'sndr_db', 'bb_wave_corr', 'bb_psd_corr', ...
     'bb_psd_rmse_db', 'bb_peak_recovered_khz', 'bb_peak_rtl_khz', ...
     'bb_fs_mhz', 'align_lag'});
writetable(T, fullfile(repo, 'recovered_analog_vs_rtl_metrics.csv'));
disp(T);
end
